...
import grails.plugin.fnordmetric.FnordMetricService
...
abstract class AbstractFnordMetricASTTransformation implements ASTTransformation {

    protected static final String HASH_CODE = '#'
    protected static final String GSTRING = '$'
    protected static final String FNORDMETRIC_SERVICE = 'fnordMetricService'
    ...
    protected boolean validateNodes(ASTNode[] astNodes, SourceUnit sourceUnit) {
        if(astNodes[0] == null || ! astNodes[0].class.isAssignableFrom(AnnotationNode)) {
            addError('Expected annotation node as first argument of fnordmetric ast transformation', astNodes[0], sourceUnit)
            return false
        }
        if(astNodes[1] == null || ! astNodes[1].class.isAssignableFrom(MethodNode)) {
            addError('Fnordmetric annotations can only be placed on methods', astNodes[1], sourceUnit)
            return false
        }
        MethodNode methodNode = (MethodNode) astNodes[1]
        if(! methodNode.getCode().class.isAssignableFrom(BlockStatement)) {
            addError('Fnordmetric annotations can only be placed on methods containing block statements. This indicates a bug in the plugin.', astNodes[1], sourceUnit)
            return false
        }
        return true
    }
     ...
     */
    private void addFnordMetricServiceProperty(ClassNode cNode, String propertyName, Class propertyType = java.lang.Object.class, Expression initialValue = null) {
    ...
    /**
     * Determine if the user missed injecting the fnordMetricService into the class with the method.
     * @param sourceUnit SourceUnit to detect and/or inject service into
     */
    protected void injectFnordMetricService(SourceUnit sourceUnit) {
        if(!((ClassNode) sourceUnit.AST.classes.toArray()[0]).properties?.any { it?.field?.name == FNORDMETRIC_SERVICE }) {
            println "Adding fnordMetricService to class ${sourceUnit.AST.classes[0].name}."
            if(!sourceUnit.AST.imports.any {it.className == ClassHelper.make(FnordMetricService).name}
                    && !sourceUnit.AST.starImports.any {it.packageName == "${ClassHelper.make(FnordMetricService).packageName}."}) {
                println "Adding namespace ${ClassHelper.make(FnordMetricService).packageName} to class ${sourceUnit.AST.classes[0].name}."
                sourceUnit.AST.addImport('FnordMetricService', ClassHelper.make(FnordMetricService))
            }
            addFnordMetricServiceProperty((ClassNode) sourceUnit.AST.classes.toArray()[0], FNORDMETRIC_SERVICE)
        }
    }
